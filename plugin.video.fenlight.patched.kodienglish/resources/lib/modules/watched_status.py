...
def clear_local_bookmarks():
	...
	try:
		...
		file_ids = dbcon.execute("SELECT idFile FROM files WHERE strFilename LIKE 'plugin://plugin.video.fenlight.patched.kodienglish/%'").fetchall()
...
def clear_local_bookmark(media_type, media_id, season='', episode=''):
	...
	try:
		...
		if media_type == 'movie': filename = 'plugin://plugin.video.fenlight.patched.kodienglish/?mode=playback.media&media_type=movie&tmdb_id=%s%%' % media_id
		elif media_type == 'episode': filename = 'plugin://plugin.video.fenlight.patched.kodienglish/?mode=playback.media&media_type=episode&tmdb_id=%s&season=%s&episode=%s%%' % (media_id, season, episode)
		else: return
