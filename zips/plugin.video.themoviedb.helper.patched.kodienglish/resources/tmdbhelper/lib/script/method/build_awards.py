...
class AwardsBuilder():
    ...
    def get_tmdb_ids(self):
        ...
        def _get_tmdb_id(tvdb_id, v):
            tmdb_id = None
            if tmdb_type == 'tv':
                tmdb_id = FindQueriesDatabase().get_tmdb_id(tmdb_type, tvdb_id=tvdb_id, query=v.get('name'), year=v.get('year'))
            else:
                tmdb_id = FindQueriesDatabase().get_tmdb_id(tmdb_type, query=v.get('name'), year=v.get('year'))
            if not tmdb_id:
...
def build_awards(**kwargs):
    ...
    path = 'special://home/addons/plugin.video.themoviedb.helper.patched.kodienglish/resources/jsondata/'
    file = 'awards.json'
