...
destination_check = translate_path('special://home/addons/plugin.video.fenlight.kodienglish/')
changelog_location = translate_path('special://home/addons/plugin.video.fenlight.kodienglish/resources/text/changelog.txt')
downloads_icon = get_icon('downloads')
addon_dir = 'plugin.video.fenlight.kodienglish'
zipfile_name = 'plugin.video.fenlight.kodienglish-%s.zip'
...
def rollback_check():
	...
	results = [i['name'].split('-')[1].replace('.zip', '') for i in results.json() if 'plugin.video.fenlight.kodienglish' in i['name'] \
