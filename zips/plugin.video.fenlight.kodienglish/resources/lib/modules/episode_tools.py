...
def play_next_from_playback():
	try:
		...
		if media_type != 'episode' or not tmdb_id or season <= 0 or episode <= 0:
			return ok_dialog(text='No Episodes Available')
		...
		if player.isPlayingVideo():
			logger('Fen Light', 'play_next_from_playback stopping current playback | tmdb_id=%s | season=%s | episode=%s' % (tmdb_id, season, episode))
			player.stop()
			for _ in range(50):
				if not player.isPlayingVideo(): break
				sleep(100)
			if player.isPlayingVideo():
				logger('Fen Light', 'play_next_from_playback stop timeout | tmdb_id=%s | season=%s | episode=%s' % (tmdb_id, season, episode))
				return notification('Next Episode Stop Timeout', 3000)
		logger('Fen Light', 'play_next_from_playback starting foreground next episode | current_tmdb_id=%s | current_season=%s | current_episode=%s | next_season=%s | next_episode=%s' % (
			tmdb_id, season, episode, url_params.get('season'), url_params.get('episode')))
		return Sources().playback_prep(url_params)
	except:
		logger('Fen Light', 'play_next_from_playback exception | error=%s' % traceback.format_exc().strip())
