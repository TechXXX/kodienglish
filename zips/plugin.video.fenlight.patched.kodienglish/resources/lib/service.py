...
class SetAddonConstants:
	def run(self):
		logger('Fen Light Patched', 'SetAddonConstants Service Starting')
		import xbmcgui, xbmcaddon, xbmcvfs
		addon_object = xbmcaddon.Addon('plugin.video.fenlight.patched.kodienglish')
		...
		return logger('Fen Light Patched', 'SetAddonConstants Service Finished')
...
class DatabaseMaintenance:
	def run(self):
		logger('Fen Light Patched', 'DatabaseMaintenance Service Starting')
		from caches.base_cache import make_databases
		make_databases()
		return logger('Fen Light Patched', 'DatabaseMaintenance Service Finished')
...
class OnUpdateChanges:
	def run(self):
		...
		except Exception as e: logger('Fen Light Patched', 'OnUpdateChanges Service Failed: %s' % str(e))
		return logger('Fen Light Patched', 'OnUpdateChanges Service Finished')
	...
	def refresh_addon_keys(self):
		...
		logger('Fen Light Patched', 'Old Trakt client ID detected. Trakt credentials reset; reauthorization required.')
	...
	def sync_magneto_undesirables_from_coco(self):
		...
		try:
			...
			try:
				...
				logger('Fen Light Patched', 'CocoScrapers undesirables settings unavailable; using Coco defaults: %s' % str(e))
			magneto_addon = xbmcaddon.Addon('script.module.magneto')
			for setting_id, value in coco_settings.items():
				magneto_addon.setSetting(setting_id, value)
		except Exception as e:
			logger('Fen Light Patched', 'Magneto undesirables settings sync failed: %s' % str(e))
		try:
			self.merge_magneto_undesirables_database()
		except Exception as e:
			logger('Fen Light Patched', 'Magneto undesirables database sync failed: %s' % str(e))
	...
	def merge_magneto_undesirables_database(self):
		...
		with sqlite3.connect(magneto_db) as magneto_con:
			...
			magneto_con.commit()
		logger('Fen Light Patched', 'Copied %s CocoScrapers undesirables entries to Magneto.' % len(rows))

class CustomFonts:
	def run(self):
		logger('Fen Light Patched', 'CustomFonts Service Starting')
		...
		return logger('Fen Light Patched', 'CustomFonts Service Finished')

class TraktMonitor:
	def run(self):
		logger('Fen Light Patched', 'TraktMonitor Service Starting')
		...
		while not monitor.abortRequested():
			...
			try:
				...
				if status == 'failed': logger('Fen Light Patched', trakt_service_string % ('Failed. Error from Trakt', next_update_string))
				else:
					if status in ('success', 'no account'): logger('Fen Light Patched', trakt_service_string % ('Success. %s' % trakt_success_line_dict[status], next_update_string))
					else: logger('Fen Light Patched', trakt_service_string % ('Success. No Changes Needed', next_update_string))# 'not needed'
					if status == 'success' and get_setting('fenlight.trakt.refresh_widgets', 'false') == 'true': run_plugin({'mode': 'kodi_refresh'})
			except Exception as e: logger('Fen Light Patched', trakt_service_string % ('Failed', 'The following Error Occured: %s' % str(e)))
...
class WidgetRefresher:
	def run(self):
		...
		while not monitor.abortRequested():
			try:
				...
				if self.next_refresh < time():
					...
					logger('Fen Light Patched', 'WidgetRefresher Service - Widgets Refreshed')
					self.set_next_refresh(time())
			except: pass
		try: del monitor
		except: pass
		...
		return logger('Fen Light Patched', 'WidgetRefresher Service Finished')
...
class FenLightMonitor(xbmc.Monitor):
	...
	def onNotification(self, sender, method, data):
		if method in ('GUI.OnScreensaverActivated', 'System.OnSleep'):
			...
			logger('OnNotificationActions', 'PAUSING Fen Light Patched Services Due to Device Sleep')
		elif method in ('GUI.OnScreensaverDeactivated', 'System.OnWake'):
			xbmcgui.Window(10000).clearProperty(pause_services_prop)
			logger('OnNotificationActions', 'UNPAUSING Fen Light Patched Services Due to Device Awake')

logger('Fen Light Patched', 'Main Monitor Service Starting')
FenLightMonitor().waitForAbort()
logger('Fen Light Patched', 'Main Monitor Service Finished')
