...
tmdb_default_api = 'd9332419f9c747636df6c20464e7b897'
...
try: current_addon = xbmcaddon.Addon()
except: current_addon = xbmcaddon.Addon('plugin.video.fenlight.patched.kodienglish')
current_addon_id = current_addon.getAddonInfo('id')
current_addon_url = 'plugin://%s/' % current_addon_id

def kodi_dialog():
	return xbmcgui.Dialog()

def addon_info(info):
	return current_addon.getAddonInfo(info)
...
def addon(addon_id=None):
	if addon_id is None: addon_id = current_addon_id
...
def run_addon(addon=None, block=False):
	if addon is None: addon = current_addon_id
...
def disable_enable_addon(addon_name=None):
	import json
	if addon_name is None: addon_name = current_addon_id
	try:
...
def update_kodi_addons_db(addon_name=None):
	import time
...
def toggle_language_invoker():
	...
	close_all_dialog()
	addon_xml = translate_path('special://home/addons/%s/addon.xml' % current_addon_id)
