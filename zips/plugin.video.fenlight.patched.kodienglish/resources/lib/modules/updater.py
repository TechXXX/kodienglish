...
destination_check = translate_path('special://home/addons/plugin.video.fenlight.patched.kodienglish/')
changelog_location = translate_path('special://home/addons/plugin.video.fenlight.patched.kodienglish/resources/text/changelog.txt')
downloads_icon = get_icon('downloads')
addon_dir = 'plugin.video.fenlight.patched.kodienglish'
zipfile_name = 'plugin.video.fenlight.patched.kodienglish-%s.zip'
heading_str = 'Fen Light Patched Updater'
error_str = 'Error'
notification_occuring_str = 'Fen Light Patched Update Occuring'
notification_available_str = 'Fen Light Patched Update Available'
notification_updating_str = 'Fen Light Patched Performing Update'
notification_rollback_str = 'Fen Light Patched Performing Rollback'
...
success_str = '[CR]Success.[CR]Fen Light Patched updated to version [B]%s[/B]'
rollback_heading_str = 'Choose Rollback Version'
success_rollback_str = '[CR]Success.[CR]Fen Light Patched rolled back to version [B]%s[/B]'
confirm_rollback_str = 'Are you sure?[CR]Version [B]%s[/B] will overwrite your current installed version.' \
						'[CR]Fen Light Patched will set your update action to [B]OFF[/B] if rollback is successful'
...
no_changes_str = 'You are running the current version of Fen Light Patched.[CR][CR]There is no new version changelog to view.'
...
def rollback_check():
	...
	results = [i['name'].split('-')[1].replace('.zip', '') for i in results.json() if 'plugin.video.fenlight.patched.kodienglish' in i['name'] \
