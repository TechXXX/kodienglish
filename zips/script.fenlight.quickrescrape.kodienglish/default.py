...
KEYMAP_FILE = "special://profile/keymaps/zz_fenlight_quickrescrape_kodienglish.xml"
...
def fen_plugin_prefix(addon_id):
    return "plugin://{}/".format(addon_id)


def fen_addon_from_url(fen_url):
    return urlparse(fen_url).netloc


def installed_fen_addon_id():
    for addon_id in FEN_ADDON_IDS:
        try:
...
def extract_fen_url(value):
    if not value:
        return ""
    for addon_id in FEN_ADDON_IDS:
        if value.startswith("plugin://{}/?".format(addon_id)) or value.startswith("plugin://{}?".format(addon_id)):
            return value
        match = re.search(r"(plugin://%s/?\?[^'\" ]+)" % re.escape(addon_id), value)
        if match:
...
def focused_fen_params():
    ...
    for labels in candidates:
        ...
        for key in ("filename", "folder", "path", "fen_options", "fen_extras"):
            ...
            params["_addon_id"] = fen_addon_from_url(fen_url)
            ...
            if params.get("mode") == "options_menu_choice" and params.get("content") == "movie":
                ...
                return {
                    ...
                    "_addon_id": params.get("_addon_id", ""),
                }
...
def fallback_params_from_labels(labels):
    ...
    params = {"mode": "playback.media", "media_type": media_type, "tmdb_id": tmdb_id, "_addon_id": installed_fen_addon_id()}
...
def clear_fen_source_cache(addon_id, media_type, tmdb_id):
    lib_path = fen_lib_path(addon_id)
    ...
    log("Cleared Fen source cache for {} {} via {}".format(media_type, tmdb_id, addon_id))


def launch_source_select(params):
    addon_id = params.get("_addon_id") or installed_fen_addon_id()
    if not addon_id:
        notify("Fen Light English is not installed")
        return
    play_params = {
        "mode": "playback.media",
    ...
    url = "{}?{}".format(fen_plugin_prefix(addon_id), urlencode(play_params))
...
def run_rescrape():
    ...
    addon_id = params.get("_addon_id") or installed_fen_addon_id()
    clear_fen_source_cache(addon_id, media_type, tmdb_id)
