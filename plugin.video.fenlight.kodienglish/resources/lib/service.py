...
class OnUpdateChanges:
	def run(self):
		...
		try:
			migrations = (
				...
				('rollback_magneto_external_scraper', self.rollback_magneto_external_scraper),
			)
	...
	def rollback_magneto_external_scraper(self):
		from caches.settings_cache import get_setting, set_setting
		current_module = get_setting('fenlight.external_scraper.module', '')
		if current_module != 'script.module.magneto': return
		try:
			import xbmcaddon
			...
			set_setting('provider.external', 'false')
			logger('Fen Light', 'Magneto rollback disabled external scraper; CocoScrapers unavailable: %s' % str(e))
			return
		set_setting('external_scraper.module', 'script.module.cocoscrapers')
		set_setting('external_scraper.name', coco_addon.getAddonInfo('name') or 'CocoScrapers Module')
		set_setting('provider.external', 'true')
		logger('Fen Light', 'External scraper rolled back from Magneto to CocoScrapers for Fen Light English.')
