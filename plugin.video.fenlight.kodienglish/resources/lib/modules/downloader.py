...
class Downloader:
	...
	def get_url_and_headers(self):
		...
		if url in (None, 'None', ''):
			if self.action == 'meta.single':
				try:
					...
					logger('Fen Light English', 'Download resolve failed | action=%s | provider=%s | media=%s | error=%s' % (
		...
		logger('Fen Light English', 'Download resolved URL | action=%s | provider=%s | source_provider=%s | media=%s | host=%s | headers=%s' % (
	...
	def download_check(self):
		...
		if not self.resp:
			logger('Fen Light English', 'Download check failed | reason=no_response | action=%s | provider=%s | url=%s' % (
		...
		if self.content < 1:
			logger('Fen Light English', 'Download check failed | reason=no_content_length | action=%s | provider=%s | url=%s | status=%s | content_type=%s | transfer_encoding=%s | accept_ranges=%s' % (
	...
	def get_response(self, size=0):
		try:
			...
			logger('Fen Light English', 'Download HTTP open failed | action=%s | provider=%s | url=%s | error=%s' % (
