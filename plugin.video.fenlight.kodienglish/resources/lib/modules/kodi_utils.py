...
tmdb_default_api = 'b78eef0a2d04954a332f4ea8ef459aad'
trakt_default_id = '1bead21224bf52d556c836f510b64408f7aa139e86965f82adde01705724cd10'
trakt_default_secret = 'e1c3a9bcd28961daca6addf755f1b41d9af34e291d3e51e1d02e50803318b018'
...
def addon_info(info):
	return xbmcaddon.Addon('plugin.video.fenlight.kodienglish').getAddonInfo(info)
...
def build_url(url_params):
	return 'plugin://plugin.video.fenlight.kodienglish/?%s' % urlencode(url_params)

def add_kodi_favourite(params):
	name, target = params.get('name') or params.get('title') or 'Fen Light KodiEnglish', params.get('path')
...
def _write_kodi_favourite_xml(name, target, thumb, is_folder):
	...
	try:
		...
		try:
			...
			if any((item.text or '') == command for item in root.findall('favourite')): return False, True
		except:
			if escape(command) in content: return False, True
...
def addon(addon_id='plugin.video.fenlight.kodienglish'):
...
def run_addon(addon='plugin.video.fenlight.kodienglish', block=False):
...
def disable_enable_addon(addon_name='plugin.video.fenlight.kodienglish'):
	import json
	try:
...
def update_kodi_addons_db(addon_name='plugin.video.fenlight.kodienglish'):
	import time
...
def toggle_language_invoker():
	...
	close_all_dialog()
	addon_xml = translate_path('special://home/addons/plugin.video.fenlight.kodienglish/addon.xml')
