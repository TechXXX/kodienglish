...
class TorBoxAPI:
	...
	def resolve_usenet_search(self, link, name, title, season, episode):
		try:
			...
			logger('Fen Light English', 'TorBox Usenet Search resolve start | name=%s | title=%s | season=%s | episode=%s' % (name, title, season, episode))
			usenet_id = self.create_usenet_transfer(link, name)
			if not usenet_id:
				logger('Fen Light English', 'TorBox Usenet Search resolve failed | reason=create_transfer | name=%s' % name)
				return None
			selected_files = self._usenet_transfer_video_files(usenet_id, extensions)
			logger('Fen Light English', 'TorBox Usenet Search transfer files | usenet_id=%s | count=%s | files=%s' % (
				usenet_id, len(selected_files), self._debug_usenet_files(selected_files)))
			if not selected_files:
				logger('Fen Light English', 'TorBox Usenet Search resolve failed | reason=no_video_files | usenet_id=%s' % usenet_id)
				return None
			if season:
				pre_filter_count = len(selected_files)
				selected_files = [i for i in selected_files if seas_ep_filter(season, episode, i['filename'])]
				if not selected_files:
					logger('Fen Light English', 'TorBox Usenet Search resolve failed | reason=episode_filter | usenet_id=%s | before=%s | target=S%02dE%02d' % (
						usenet_id, pre_filter_count, int(season), int(episode)))
			else:
				if self._m2ts_check(selected_files):
					logger('Fen Light English', 'TorBox Usenet Search resolve failed | reason=m2ts_folder | usenet_id=%s' % usenet_id)
					return None
			...
			logger('Fen Light English', 'TorBox Usenet Search selected file | usenet_id=%s | filename=%s | size=%s' % (
				usenet_id, chosen_file.get('filename'), chosen_file.get('size')))
			file_key = chosen_file['url']
			file_url = self.unrestrict_usenet(file_key)
			logger('Fen Light English', 'TorBox Usenet Search requestdl | usenet_id=%s | success=%s' % (usenet_id, bool(file_url)))
			...
			logger('Fen Light English', 'TorBox Usenet Search resolve exception | usenet_id=%s | error=%s' % (usenet_id, str(e)))
	...
	def resolve_magnet(self, magnet_url, info_hash, store_to_cloud, title, season, episode):
		try:
			...
			else: logger('Fen Light English', 'TorBox resolve keeping transfer alive | torrent_id=%s | title=%s | success=%s' % (torrent_id, title, bool(file_url)))
			return file_url
		except:
	...
	def display_magnet_pack(self, magnet_url, info_hash, download=False):
		...
		try:
			...
			torrent_files = [{'link': '%d,%d' % (torrent_id, item['id']), 'filename': item['short_name'], 'size': item['size']} \
							for item in torrent_files['data']['files'] if item['short_name'].lower().endswith(tuple(extensions))]
			if download:
				logger('Fen Light English', 'TorBox pack download keeping transfer alive | torrent_id=%s | files=%s' % (torrent_id, len(torrent_files)))
			else:
